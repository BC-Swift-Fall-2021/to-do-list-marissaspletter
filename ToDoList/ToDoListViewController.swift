...
//  ToDoList
//
//  Created by Marissa Spletter on 9/26/21.
//
...
class ToDoListViewController: UIViewController {
    ...
    //var toDoArray = ["one","two","three","four"]
    ...
    func autherizeLocalNotifications() {
        UNUserNotificationCenter.current().requestAuthorization(options: [.alert,.sound,.badge]) { (granted, error)
            ...
            if granted {
                print("Notifications Authorization garnted")
            } else {
                print("Denied")
                //TODO: put in a alert telling user what to do
            }
        }
    }
    ...
    func setCalendarNotification (title: String, subtitle: String, body: String, badgeNumber: NSNumber?, sound: UNNotificationSound?, date: Date) -> String {
        ...
        //register request w/notification center
        UNUserNotificationCenter.current().add(request) { (error) in
            if let error = error {
                print("ERROR: \(error.localizedDescription) yikes adding a error notification went wrong")
                
            } else {
                print("Notification center \(notificationID)")
            }
        }
